...
namespace react_template_identity.Services
{
    public class UserService : IUserService
    {
        ...
        {
            return await Get(username, cancellationToken) != null;
        }

        public async Task<User> Create(UserModel model, CancellationToken cancellationToken)
        {
            if (await Exists(model.Username, cancellationToken))
            {
                return null;
            }
        ...
        public async Task ConfirmEmail(string username, CancellationToken cancellationToken)
        {
            var user = await Get(username, cancellationToken);

            if (user is null)
            {
                await Task.FromException(new ArgumentException($"Użytkownik nie istnieje. Szczegóły: {username}"));
            }
            ...
            if (!identityResult.Succeeded)
            {
                ...
                await Task.FromException(new ArgumentException($"Problem podczas zmiany hasła u " +
                    $"użytkownika: {username}. Szczegóły: {string.Join(", ", errors)}"));
            }
        }

        public async Task ChangePassword(string username, UserPasswordModel model, CancellationToken cancellationToken)
        {
            var user = await Get(username, cancellationToken);
        ...
        public async Task ResetPassword(string username, UserPasswordModel model, CancellationToken cancellationToken)
        {
            var user = await Get(username, cancellationToken);
            ...
            var identityResult = await _userManager.ResetPasswordAsync(user, token, model.NewPassword);
